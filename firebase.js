  ...
  try {
    ...
    await setDoc(
      doc(db, "companys", companyID, "clockIn", auth.currentUser.email),
      {
        ...
        company: company,
        companyID: companyID,
        recentClockOut: serverTimestamp(),
        totalHoursToday: minutesConverted,
      },
...
// Get most recent clock in and clock out
export function getRecentClockInAndOut({ companyID, setRecentClockInAndOut }) {
  onSnapshot(
    query(
      collection(db, "companys", companyID, "clockIn"),
      orderBy("recentClockIn", "desc"),
      limit(50)
    ),
    (querySnapshot) => {
      const recentClockInAndOut = [];
      querySnapshot.forEach((snap) => {
        recentClockInAndOut.push(snap.data());
      });
      setRecentClockInAndOut(recentClockInAndOut);
    }
  );
}
//for each users in clockIn get allTimeTracking
...
export function getAllTimeTracking({
  ...
  onSnapshot(
    query(collection(db, "companys", companyID, "clockIn"), limit(50)),
    (querySnapshot) => {
      const allHoursAndUsers = [];

      querySnapshot.forEach((snapUser) => {
        //for each snap get allTimeTracking
        const clockIn = [];
        const clockOut = [];
        onSnapshot(
          query(
          ...
          (querySnapshot) => {
            querySnapshot.forEach((snap) => {
              if (snap.get("clockInBool")) {
                //console.log(snap.get("clockIn"));
                clockIn.push(snap.get("clockIn"));
              } else if (snap.get("clockOutBool")) {
                console.log("clockOutBool", snap.get("clockOut"));
                clockOut.push(snap.get("clockOut"));
              }
            });
            ...
            clockIn.forEach((item, index) => {
              ...
              //if the clock out is not empty
              var clockInSeconds = 0;
              if (item.seconds) {
                clockInSeconds = item.seconds;
              }

              var difference;
              //if there is a clock out

              if (clockOut[index]) {
                difference = clockOut[index].seconds - clockInSeconds;
              }
            ...
            allHoursAndUsers.push({
              user: snapUser.id,
              totalHours: hoursRounded,
            });

            allDifference = [];
            setAllTimeTracking(allHoursAndUsers);
          }
        );
      });
    }
